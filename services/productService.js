...
const create = async (data, file, err) => {
  ...
  if (file) {
    newProduct.photoUrl = `http://localhost:8080/uploads/${file.filename}`;
  }
...
const update = async (slug, data, file, err) => {
  ...
  const updatedProduct = await prisma.product.update({
    ...
    data: {
      ...
      photo: file ? `/uploads/${file.filename}` : existingProduct.photo,
    },
  });
  ...
  if (file) {
    updatedProduct.photoUrl = `http://localhost:8080/uploads/${file.filename}`;
  }
